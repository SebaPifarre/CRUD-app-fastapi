...
def get_posts(db: Session = Depends(get_db), current_user:int = Depends(oauth2.get_current_user),
    ...
    # cursor.execute("""SELECT * FROM posts""")     I'm commenting out all the SQL queries to replace them with the ORM connection.
...
def get_post(id: int, db: Session = Depends(get_db), current_user:int = Depends(oauth2.get_current_user)):  # Validation of id
